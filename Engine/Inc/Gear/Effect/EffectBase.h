...
namespace Gear::Effect
{
	...
	public:
		...
		EffectBase(ResourceManager& refResManager, const uint32_t width, const uint32_t height, const DXGI_FORMAT rtvFormat);

		EffectBase(ResourceManager& refResManager, const uint32_t width, const uint32_t height, const DXGI_FORMAT resFormat, const uint32_t arraySize, const uint32_t mipLevels, const bool isTextureCube,
