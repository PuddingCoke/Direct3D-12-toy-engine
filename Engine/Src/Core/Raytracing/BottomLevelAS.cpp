...
void BottomLevelAS::addGeometryObject(const GeometryObject object)
{
	...
	{
		LOGERROR(L"input vertex buffer does not need state tracking however its internal state does not have D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE");
	}
	...
	{
		LOGERROR(L"input index buffer does not need state tracking however its internal state does not have D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE");
	}
	...
	{
		LOGERROR(L"input transform buffer does not need state tracking however its internal state does not have D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE");
	}
