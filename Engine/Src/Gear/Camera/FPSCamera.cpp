			...
			moveEventID = Input::Mouse::addMoveEvent([this](const uint32_t)
				{
					...
					{
						const DirectX::XMMATRIX rotMat = DirectX::XMMatrixRotationAxis(this->up, Input::Mouse::getDeltaX() / 360.f);
						...
						const float destAngle = lookUpAngle - Input::Mouse::getDeltaY() / 360.f;

						float rotAngle = -Input::Mouse::getDeltaY() / 360.f;
